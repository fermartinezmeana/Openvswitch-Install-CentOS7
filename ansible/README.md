...
You **need change the network configuration** and the Openvswitch version. To do this, modify the file `all` located in the folder `group_vars`. To set the `bridge_exterior` you need find out your ip address, netmask and gateway. Use commands `ip a` and `ip r` to know them
